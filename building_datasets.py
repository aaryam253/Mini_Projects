...
# sample a few users and get their recommended books --> used to give an indication of the accuracy of the model
def sample_recommendation(model, interactions, item_features, user_ids):

	n_users, n_items = interactions.shape

	for user_id in user_ids:
		known_positives = item_features[interactions.tocsr()[user_id].indices]
...
train, test = random_train_test_split(interactions, test_percentage=0.01, random_state=None)
...
sample_recommendation(model, interactions, item_features, [124,128,1231,7682,8819,9019])
